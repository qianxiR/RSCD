...
from .decoder import ChangeDecoder # 导入变化检测解码器
from .network_utils import weight_init # 导入权重初始化函数
...
class Encoder(nn.Module):
    ...
    def base_forward(self, x: torch.Tensor, output_final: bool=False) -> List[torch.Tensor]:
        ...
        Args:
            ...
            output_final (bool): 如果为 True，则只返回最后一个 Stage 的输出。
...
class Trainer(nn.Module):
    ...
    def update_bcd(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        # 2. 提取每个 stage 的第一个 perception frame 特征

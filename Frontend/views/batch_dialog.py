...
import gc
from Controller.local_service import submit_detection, get_task_status, check_connection
# 本地检测服务（取代 api_client）
...
# 线程池单例统一从 common 子包导入（消除与 raster_batch 的重复实现）
# 历史问题: 本文件用 GLOBAL_THREAD_POOL，raster_batch 用 THREAD_POOL，两份独立实现
from Frontend.views.common.thread_pool import (
    get_thread_pool, cleanup_thread_pool,
    CPU_COUNT, DEFAULT_THREAD_POOL_SIZE,
)
# 向后兼容: 部分代码可能仍引用 GLOBAL_THREAD_POOL 名称
GLOBAL_THREAD_POOL = None

# 模块组织: 四大块 —— UI 构建、变化检测执行、渔网分割执行、工作线程
...
#变化检测开始
    ...
    def start_batch_processing(self):
        ...
        # 清空日志并添加开始消息
        self.preview_list.clear()
        self._add_log("开始批量影像变化检测处理，这可能需要一点时间...")
        ...
        try:
            ...
            self.batch_worker = BatchImageProcessingWorker(data, None)
...
#变化检测结束
    ...
    def start_grid_cropping(self):
        ...
        try:
            ...
            # 获取网格大小
            try:
                # 网格解析逻辑统一委托给 common.utils.parse_grid_size
                from Frontend.views.common.utils import parse_grid_size
                grid_size_text = self.grid_size_input.text().strip()
                rows, cols = parse_grid_size(grid_size_text)
...
class BatchProcessing:
    ...
    def __init__(self, navigation_functions):
        ...
        if self.api_available:
            self.navigation_functions.log_message("变化检测服务已加载")
...
# 注: 线程池清理已由 Frontend.views.common.thread_pool 模块在加载时 atexit 注册，
# 此处无需重复定义 cleanup_thread_pool 与 atexit.register


class BatchImageProcessingWorker(QObject):
    """批量影像变化检测工作线程类（本地提交 + 轮询）"""

    def __init__(self, data, endpoint_url):
        """初始化批处理工作线程（endpoint_url 保留参数以兼容旧签名，本地化后未使用）"""
        super().__init__()
        self.data = data
    ...
    def run(self):
        ...
        try:
            ...
            task_id = submit_detection(before_path, after_path, output_path, batch=True)

            # 进程内轮询至终态
            result = {"status": "error", "message": "轮询未启动"}
            ...
            import traceback
            error_info = traceback.format_exc()
            self.signals.error.emit(f"批处理任务出错: {str(e)}\n{error_info}")
        finally:
            self.is_running = False

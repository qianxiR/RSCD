...
class GridCropping:
    ...
    def select_image_file(self, title="选择影像文件"):
        """选择影像文件（统一支持普通图像与栅格影像）

        入参: title — 文件选择对话框标题
        方法: 弹出文件选择对话框，过滤器包含 png/jpg/jpeg/tif/tiff
        出参: 选择的文件路径，未选择返回空字符串
        """
        file_filter = "影像文件 (*.png *.jpg *.jpeg *.tif *.tiff);;所有文件 (*.*)"

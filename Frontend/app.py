...
from Frontend.widgets import NavigationFunctions, ZoomableLabel
from Frontend.theme import ThemeManager
from Frontend.views.image_import import ImportBeforeImage
from Frontend.views.image_import import ImportAfterImage
...
from Frontend.widgets import NavigationFunctions, ZoomableLabel
from Frontend.theme import ThemeManager
from Frontend.views import ImportBeforeImage,ImportAfterImage,ExecuteChangeDetectionTask,ClearTask,GridCropping,ImageDisplay
...
class HomePage(QWidget):
    ...
    def get_image_mode_button(self):
        """获取进入主界面按钮（统一入口，保留方法名以兼容外部调用）"""
        return self.btn_image_mode

    def get_raster_mode_button(self):
        """获取进入主界面按钮（与 image 入口同一按钮，保留方法名以兼容外部调用）"""
        return self.btn_image_mode
    ...
    def handle_task_completion(self, success, task_name):
        ...
        # 清理任务完成后即可启用按钮（本地化后无需等待 Docker/API）
        if self.cleanup_done:
            if self.cleanup_success:
                ...
                self.update_loading_message("初始化失败: 清理目录失败。")
    ...
    def init_ui(self):
        ...
        # 系统标题
        self.title_label = QLabel("遥感影像变化检测系统 V1.0") # Store reference
        ...
        # 单一进入按钮（取消图像版/影像版二元划分）
        self.btn_image_mode = QPushButton("开始使用")
        self.btn_image_mode.setObjectName("imageVersionButton")
        self.btn_image_mode.setFont(QFont("Microsoft YaHei UI", 10, QFont.Bold))
        self.btn_image_mode.setMinimumSize(160, 44)

        # 影像版按钮引用指向同一按钮（保留属性以兼容外部访问）
        self.btn_raster_mode = self.btn_image_mode
        ...
        # 进入说明
        self.mode_description = QLabel("点击下方按钮进入变化检测主界面")
        ...
        # 统一流程说明
        self.image_mode_tip = QLabel("系统自动识别影像坐标：有坐标保留地理参考并导出矢量，无坐标输出普通图像")
        ...
        self.raster_mode_tip = self.image_mode_tip
...
class RemoteSensingApp(QMainWindow):
    ...
    def initialize_all_components(self):
        ...
        try:
            ...
            if not getattr(self, "modules_initialized", False):
    ...
    def connect_home_page_signals(self):
        ...
        # 单一进入按钮
        self.home_page.get_image_mode_button().clicked.connect(switch_to_main)
    ...
    def switch_to_main_page(self):
        ...
        try:
            ...
            if hasattr(self, "previous_mode") and self.previous_mode is not None:
                if hasattr(self, "clear_task") and self.clear_task is not None:
                    self.clear_task.clear_interface()
                    QApplication.processEvents()
            self.previous_mode = "main"

            self.update_ui_for_mode("main")
            self.setWindowTitle("遥感影像变化检测系统 V1.0")
            self.stacked_widget.setCurrentIndex(1)

            if not self.statusBar().currentMessage():
                self.statusBar().showMessage("主界面已加载", 2000)
    ...
    def update_ui_for_mode(self, mode):
        ...
        if hasattr(self, 'group_output') and self.group_output is not None:
            self.group_output.setTitle("变化检测结果")

        if hasattr(self, 'navigation_functions') and self.navigation_functions is not None:
            self.navigation_functions.log_message("已进入变化检测主界面")
    ...
    def show_help(self):
        ...
        <body>
            ...
            <p>本系统用于检测两个时相影像之间的变化，自动识别影像坐标：带地理坐标的影像将保留坐标系并导出矢量，无坐标影像输出普通图像。</p>

            <h3>基本操作流程：</h3>
            <ol>
                <li><b>渔网分割</b>: 对大尺寸影像进行裁剪，便于分析处理。</li>
                <li><b>导入前时相影像</b>: 导入较早时期的影像（支持 PNG/JPG/TIFF）。</li>
                <li><b>导入后时相影像</b>: 导入较新时期的影像。</li>
                <li><b>开始解译</b>: 运行变化检测算法分析两个时相影像的差异。</li>
                <li><b>批量处理</b>: 批量处理多个影像对。</li>
            </ol>

            <h3>功能按钮说明：</h3>
            <ul>
                <li><b>首页</b>: 返回系统首页。</li>
                <li><b>渔网分割</b>: 将大尺寸影像分割成若干小块。</li>
                <li><b>导入前/后时相影像</b>: 导入待对比的两个时相影像。</li>
                <li><b>开始解译</b>: 运行变化检测算法。</li>
                <li><b>批量处理</b>: 批量处理多对影像。</li>
                <li><b>模型训练</b>: 启动模型训练。</li>
    ...
    def show_training_dialog(self):
        ...
        入参: 无
        方法: 懒加载 TrainingModule → 调 show_training_dialog
        出参: 无

        做什么: 主窗口"模型训练"按钮的派发入口
        为什么: 仿 show_batch_processing 的懒加载模式，避免启动时初始化训练模块
        """
        if not hasattr(self, 'training_module') or self.training_module is None:
            QApplication.setOverrideCursor(Qt.WaitCursor)
            try:
                from Frontend.views.training_dialog import TrainingModule
                self.training_module = TrainingModule(self.navigation_functions)
            finally:
                QApplication.restoreOverrideCursor()

        self.training_module.show_training_dialog()
    ...
    def apply_button_styles(self):
        ...
        # 模型训练按钮使用主要按钮样式（与批量处理同级）
        self.btn_train.setStyleSheet(ThemeManager.get_primary_button_style(self.is_dark_theme))
    ...
    def connect_buttons(self):
        ...
        self.btn_train.clicked.connect(self.show_training_dialog)
    ...
    def create_button_group(self, parent_layout):
        ...
        for btn in [self.btn_crop, self.btn_import, self.btn_import_after, self.btn_begin, self.btn_batch, self.btn_train, self.btn_theme, self.btn_clear, self.btn_help]:
